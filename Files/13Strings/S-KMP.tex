...
通常，我們使用的 PMT 陣列會根據索引調整，使得 $\text{PMT}[k]$ 對應的是長度為 $k$ 的前綴的最長共同前後綴的長度。此時 PMT 的大小為 $m$， $\text{PMT}[0]$ 沒有實際意義或定義為 -1。在 C++ 實作中，我們通常稱其為 \textbf{lps} (longest proper prefix suffix) 陣列，且 \verb|lps[i]| 表示 $P[0 \dots i-1]$ 這個長度為 $i$ 的前綴的最長共同前後綴的長度。

例如，對於模式串 $P = \text{ABABCABAB}$ (長度 $m=9$)
...
模式串 $P = \text{AAACAAAAAC}$ ($m=10$)
初始化：\texttt{lps} 陣列大小為 10，\texttt{len = 0}，\texttt{i = 1}。

\begin{enumerate}
    \item \texttt{i = 1}, \texttt{P[1] = 'A'}, \texttt{P[len] = P[0] = 'A'}
    \texttt{P[1] == P[0]}。
    \texttt{len} 變為 1。\texttt{lps[1] = 1}。\texttt{i} 變為 2。
    \texttt{lps = [0, 1, ?, ?, ?, ?, ?, ?, ?, ?]}

    \item \texttt{i = 2}, \texttt{P[2] = 'A'}, \texttt{P[len] = P[1] = 'A'}
    \texttt{P[2] == P[1]}。
    \texttt{len} 變為 2。\texttt{lps[2] = 2}。\texttt{i} 變為 3。
    \texttt{lps = [0, 1, 2, ?, ?, ?, ?, ?, ?, ?]}

    \item \texttt{i = 3}, \texttt{P[3] = 'C'}, \texttt{P[len] = P[2] = 'A'}
    \texttt{P[3] != P[2]}。
    \texttt{len != 0} (len=2)。所以 \texttt{len = lps[len-1] = lps[1] = 1}。
    \texttt{lps = [0, 1, 2, ?, ?, ?, ?, ?, ?, ?]} (len 變回 1)

    \item \texttt{i = 3}, \texttt{P[3] = 'C'}, \texttt{P[len] = P[1] = 'A'}
    \texttt{P[3] != P[1]}。
    \texttt{len != 0} (len=1)。所以 \texttt{len = lps[len-1] = lps[0] = 0}。
    \texttt{lps = [0, 1, 2, ?, ?, ?, ?, ?, ?, ?]} (len 變回 0)

    \item \texttt{i = 3}, \texttt{P[3] = 'C'}, \texttt{P[len] = P[0]} (len=0，這表示比較 \texttt{P[3]} 和 \texttt{P[0]})
    \texttt{P[3] != P[0]}。
    \texttt{len == 0}。
    \texttt{lps[3] = 0}。\texttt{i} 變為 4。
    \texttt{lps = [0, 1, 2, 0, ?, ?, ?, ?, ?, ?]}

    \item \texttt{i = 4}, \texttt{P[4] = 'A'}, \texttt{P[len] = P[0] = 'A'}
    \texttt{P[4] == P[0]}。
    \texttt{len} 變為 1。\texttt{lps[4] = 1}。\texttt{i} 變為 5。
    \texttt{lps = [0, 1, 2, 0, 1, ?, ?, ?, ?, ?]}

    \item \texttt{i = 5}, \texttt{P[5] = 'A'}, \texttt{P[len] = P[1] = 'A'}
    \texttt{P[5] == P[1]}。
    \texttt{len} 變為 2。\texttt{lps[5] = 2}。\texttt{i} 變為 6。
    \texttt{lps = [0, 1, 2, 0, 1, 2, ?, ?, ?, ?]}

    \item \texttt{i = 6}, \texttt{P[6] = 'A'}, \texttt{P[len] = P[2] = 'A'}
    \texttt{P[6] == P[2]}。
    \texttt{len} 變為 3。\texttt{lps[6] = 3}。\texttt{i} 變為 7。
    \texttt{lps = [0, 1, 2, 0, 1, 2, 3, ?, ?, ?]}

    \item \texttt{i = 7}, \texttt{P[7] = 'A'}, \texttt{P[len] = P[3] = 'C'}
    \texttt{P[7] != P[3]}。
    \texttt{len != 0} (len=3)。所以 \texttt{len = lps[len-1] = lps[2] = 2}。
    \texttt{lps = [0, 1, 2, 0, 1, 2, 3, ?, ?, ?]} (len 變回 2)

    \item \texttt{i = 7}, \texttt{P[7] = 'A'}, \texttt{P[len] = P[2] = 'A'}
    \texttt{P[7] == P[2]}。
    \texttt{len} 變為 3。\texttt{lps[7] = 3}。\texttt{i} 變為 8。
    \texttt{lps = [0, 1, 2, 0, 1, 2, 3, 3, ?, ?]}

    \item \texttt{i = 8}, \texttt{P[8] = 'A'}, \texttt{P[len] = P[3] = 'C'}
    \texttt{P[8] != P[3]}。
    \texttt{len != 0} (len=3)。所以 \texttt{len = lps[len-1] = lps[2] = 2}。
    \texttt{lps = [0, 1, 2, 0, 1, 2, 3, 3, ?, ?]} (len 變回 2)

    \item \texttt{i = 8}, \texttt{P[8] = 'A'}, \texttt{P[len] = P[2] = 'A'}
    \texttt{P[8] == P[2]}。
    \texttt{len} 變為 3。\texttt{lps[8] = 3}。\texttt{i} 變為 9。
    \texttt{lps = [0, 1, 2, 0, 1, 2, 3, 3, 3, ?]}

    \item \texttt{i = 9}, \texttt{P[9] = 'C'}, \texttt{P[len] = P[3] = 'C'}
    \texttt{P[9] == P[3]}。
    \texttt{len} 變為 4。\texttt{lps[9] = 4}。\texttt{i} 變為 10 (結束)。
    \texttt{lps = [0, 1, 2, 0, 1, 2, 3, 3, 3, 4]}
\end{enumerate}
...
void KMPSearch(const string& txt, const string& pat) {
    ...
    if (m > n) {
        cout << "Pattern length is greater" << endl;
        return;
    }
